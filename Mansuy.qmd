...
```{r}
grades <- grades |>
  mutate(course_id = as.integer(as.character(course_id)))
grades <- grades |>
  mutate(course_id = as.integer(as.character(course_id)))
...
students_pass <- grades_long |>
  group_by(id, group) |>
  summarize(
    final_grade = mean(avg_grade, na.rm = TRUE),                    
    min_course = min(avg_grade, na.rm = TRUE),                      
    module1_avg = mean(avg_grade[module == 1], na.rm = TRUE),       
    module2_avg = mean(avg_grade[module == 2], na.rm = TRUE),        
    pass = (min_course >= 5) & (module1_avg >= 10) & (module2_avg >= 10),
    .groups = "drop"
  ) |>
...
students_almost_pass <- students_pass |>
  filter(!pass & final_grade >= 10)


num_students <- nrow(students_almost_pass)

cat("The number of students who do not pass but have a final grade >= 10 is", num_students, ".\n")


students_almost_pass |>
  slice_head(n = 5) |>
  knitr::kable(digits = 2)
...
ggplot(pass_rate_by_group, aes(x = factor(group), y = pass_rate, fill = factor(group))) +
...
failures_by_course <- grades_long |>
  filter(avg_grade < 5) |>
  group_by(course) |>
  summarize(
    num_failures = n(),
...
most_responsible_course <- failures_by_course |> slice_max(num_failures, n = 1) |> pull(course)

cat("The course most responsible for student failures due to grades below 5 is:", 
...
```{r}
students_pass_age <- students_pass |>
  left_join(students |> select(id, birth_date), by = "id") |>
  mutate(
    age_years = time_length(today() - birth_date, unit = "year") |> floor()
  )

success_by_age <- students_pass_age |>
  group_by(age_years) |>
  summarize(
    success_rate = mean(pass) * 100,
    n_students = n(),
    .groups = "drop"
  )
...
ggplot(success_by_age, aes(x = age_years, y = success_rate)) +
